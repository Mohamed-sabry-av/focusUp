...
const CATEGORIES = ['CODING', 'WRITING', 'STUDYING', 'DESIGN', 'ADMIN', 'OTHER'];
    ...
    <div className="flex min-h-screen w-full items-center justify-center bg-gray-50 dark:bg-zinc-950 p-4">
      <Card className="w-full max-w-md">
        <form onSubmit={handleSubmit(onSubmit)}>
          <CardHeader>
            <CardTitle className="text-2xl font-bold">Welcome to FocusUp</CardTitle>
            <CardDescription>Step {step} of 3</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            {serverError && <div className="p-3 text-sm bg-red-100 text-red-800 rounded">{serverError}</div>}
            
            {step === 1 && (
              <div className="space-y-4">
                <h3 className="font-semibold text-lg">Confirm your timezone</h3>
                <p className="text-sm text-gray-500">We need this to schedule your sessions accurately.</p>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <Controller
                    ...
                    render={({ field }) => (
                      <>
                        {CATEGORIES.map((cat) => (
                          <div key={cat} className="flex items-center space-x-2 pt-2">
                            <Checkbox 
                              id={`cat-${cat}`}
            ...
            {step === 3 && (
              <div className="space-y-4">
                <h3 className="font-semibold text-lg">Preferred session lengths</h3>
                <p className="text-sm text-gray-500">Choose the standard session durations you prefer.</p>
                <div className="space-y-3">
                  <Controller
                    ...
                    render={({ field }) => (
                      <>
                        {LENGTHS.map((len) => (
                          <div key={len} className="flex items-center space-x-2 pt-2">
                            <Checkbox 
                              id={`len-${len}`}
                ...
                {errors.preferredLength && <p className="text-sm text-red-500">{errors.preferredLength.message}</p>}
              </div>
            )}
          </CardContent>
