    ...
    <div className="min-h-screen flex flex-col bg-[#fcf9f8] text-[#1c1b1b] font-sans antialiased animate-in fade-in duration-500">
      ...
      <header className="w-full px-8 py-8 md:py-10 max-w-7xl mx-auto flex justify-center md:justify-start items-center">
        <div className="flex items-center gap-2">
          ...
          <span className="font-extrabold text-2xl tracking-tight text-[#003076]">FocusUP</span>
        </div>
      </header>
          ...
          <div className="hidden lg:flex lg:col-span-5 relative overflow-hidden bg-[#0245a3]">
            ...
            <div className="relative z-10 p-12 flex flex-col justify-end h-full text-white bg-gradient-to-t from-[#0245a3] to-transparent">
              <h1 className="text-4xl font-extrabold leading-tight mb-4 tracking-tight">
                Deep work is a journey,<br/>not a destination.
              </h1>
              <p className="text-[#9eb9ff] text-lg font-medium leading-relaxed">
                Join a community dedicated to the art of focus and cognitive endurance.
              </p>
            </div>
          </div>
          ...
          <div className="lg:col-span-7 p-8 md:p-14 lg:p-16 flex flex-col justify-center bg-white">
            <div className="max-w-md mx-auto w-full">
              ...
              <div className="mb-10 text-center md:text-left">
                <h2 className="text-3xl font-extrabold tracking-tight text-slate-800 mb-2">Create your account</h2>
                <p className="text-slate-500 font-medium">Enter your details to begin your focus session.</p>
              </div>
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <label className="text-xs font-bold text-slate-500 px-1 uppercase tracking-wider block">Display Name</label>
                    <input 
                      ...
                      className={cn(
                        ...
                        errors.displayName ? "border-red-300 focus:border-red-500 bg-white" : "border-transparent focus:border-[#003076] hover:border-slate-200 bg-slate-50 focus:bg-white"
                      )}
                      placeholder="Jane Doe" 
                    />
                    {errors.displayName && <p className="text-xs text-red-500 font-medium ml-1 animate-in fade-in">{errors.displayName.message}</p>}
                  </div>

                  <div className="space-y-1.5">
                    <label className="text-xs font-bold text-slate-500 px-1 uppercase tracking-wider block">Username</label>
                    <input 
                      {...register('username')}
                      className={cn(
                        "w-full px-4 py-3.5 rounded-xl border-2 transition-all duration-200 outline-none text-slate-800 font-medium placeholder:text-slate-400 placeholder:font-normal bg-slate-50",
                        errors.username ? "border-red-300 focus:border-red-500 bg-white" : "border-transparent focus:border-[#003076] hover:border-slate-200 bg-slate-50 focus:bg-white"
                      )}
                      placeholder="janedoe" 
                    />
                    {errors.username && <p className="text-xs text-red-500 font-medium ml-1 animate-in fade-in">{errors.username.message}</p>}
                  </div>
                </div>

                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-slate-500 px-1 uppercase tracking-wider block">Email Address</label>
                  <input 
                    {...register('email')}
                    type="email"
                    className={cn(
                      "w-full px-4 py-3.5 rounded-xl border-2 transition-all duration-200 outline-none text-slate-800 font-medium placeholder:text-slate-400 placeholder:font-normal bg-slate-50",
                      errors.email ? "border-red-300 focus:border-red-500 bg-white" : "border-transparent focus:border-[#003076] hover:border-slate-200 bg-slate-50 focus:bg-white"
                    )}
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-slate-500 px-1 uppercase tracking-wider block">Password</label>
                  <div className="relative group transition-all">
                    <input 
                      ...
                      className={cn(
                        ...
                        errors.password ? "border-red-300 focus:border-red-500 bg-white" : "border-transparent focus:border-[#003076] hover:border-slate-200 bg-slate-50 focus:bg-white"
                      )}
                      ...
                      {[1, 2, 3, 4].map((level) => (
                        <div 
                          ...
                          className={cn(
                            ...
                            strengthScore >= level 
                              ? strengthScore <= 2 ? "bg-amber-400" : strengthScore === 3 ? "bg-blue-400" : "bg-emerald-500"
                              : "bg-slate-200"
                          )}
                        />
                      ))}
              ...
              <div className="mt-10 text-center">
                <p className="text-sm font-medium text-slate-500">
                  ...
                  <Link className="text-[#003076] font-bold hover:underline decoration-2 underline-offset-4 ml-1.5 transition-all" href="/login">
                    Log In
                  </Link>
                </p>
              </div>
      ...
      <footer className="w-full mt-auto">
        <div className="border-t border-slate-200/60 flex flex-col md:flex-row justify-between items-center px-8 py-8 md:py-10 max-w-7xl mx-auto gap-4">
          ...
          <div className="flex gap-6 md:gap-8 flex-wrap justify-center">
            <a className="text-xs font-semibold text-slate-500 hover:text-[#003076] transition-all" href="#">Privacy Policy</a>
            <a className="text-xs font-semibold text-slate-500 hover:text-[#003076] transition-all" href="#">Terms of Service</a>
            <a className="text-xs font-semibold text-slate-500 hover:text-[#003076] transition-all" href="#">Contact Us</a>
          </div>
          <div className="text-xs font-semibold text-slate-400">© 2024 FocusUP. Your Digital Sanctuary.</div>
        </div>
      </footer>
    </div>
  );
