...
export class SessionsController {
     ...
     */
    static async getLivekitToken(req, res, next) {
        try {
            if (!req.user)
     ...
     */
    static async setGoal(req, res, next) {
        try {
            if (!req.user)
                throw new AppError('Not authenticated', 401);
            const { sessionId } = req.params;
     ...
     */
    static async createReflection(req, res, next) {
        try {
            if (!req.user)
                throw new AppError('Not authenticated', 401);
            const { sessionId, text, rating } = req.body;
            if (!sessionId || !text) {
